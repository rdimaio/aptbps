...
def opt_encode(x, bps_arrangement='random', n_bps_points=512, radius=1.5, return_idx=False, nn_algo="kd_tree",
           leaf_size=16, verbose=1, random_seed=13, x_features=None, custom_basis=None, n_jobs=-1):
    """Converts point clouds to basis point set (BPS) representation, multi-processing version
    ...
    if n_jobs == 1:
        ...
        bps_encode_func = partial(encode, bps_arrangement=bps_arrangement, n_bps_points=n_bps_points, radius=radius,
                                  verbose=verbose, leaf_size=leaf_size, random_seed=random_seed, return_idx=return_idx,
                                  x_features=x_features, custom_basis=custom_basis, n_jobs=1, nn_algo=nn_algo)
...
def adaptive_encode(x, bps_arrangement='random', n_bps_points=512, n_parts=8, radius=1.5, bps_cell_type='dists',
    ...
    if n_jobs == 1:
        ...
        for fid in fid_lst:
            ...
            for i in range(1, n_parts):
                ...
                basis_part_idxs.append(i*n_bps_points//n_parts)
            ...
            for i in range(0, n_parts):
                ...
                #rand_idxs = np.random.choice(len(basis_density_cat[bps_start_idx:bps_end_idx, 0:3]), size=n_curr_basis_points,replace=False)
                #rand_idxs = np.random.choice(
                #    n_bps_points, size=n_curr_basis_points, replace=False)
                #curr_basis_points = basis_set[0:n_curr_basis_points]
            ...
            #for i in range(0, n_parts):
            #    bps_parts[i] = bps_parts[i].reshape(-1,3)
            ...
            # If the number of total points still isn't n_bps_points, add neighbours for least dense partition
            #if bps_parts_concat.shape[1] is not n_bps_points:
            #    curr_basis_points = basis_set[0:(n_bps_points - bps_parts_concat.shape[0])]
#
            #    # set up tree for nearest neighbors
            #    nbrs = NearestNeighbors(n_neighbors=1, leaf_size=16, algorithm="kd_tree").fit(
            #        input_density_cat[0:(num_points//n_parts), 0:3])
#
            #    # fid_dist, npts_ix = nbrs.kneighbors(curr_basis_points, n_neighbors=(1*n_parts-1*i), return_distance=True)
            #    fid_dist, npts_ix = nbrs.kneighbors(
            #        curr_basis_points, n_neighbors=1, return_distance=True)
            #    
            #    bps_parts_concat = np.vstack(
            #        (bps_parts_concat, input_density_cat[npts_ix, 0:3]))
            #    fid_dists_concat = np.vstack((fid_dists_concat, fid_dist))
...
def viz_encode(x, bps_arrangement='random', n_bps_points=512, n_parts=8, radius=1.5, bps_cell_type='dists',
           verbose=1, random_seed=13, x_features=None, custom_basis=None, n_jobs=-1):
    """Converts point clouds to basis point set (BPS) representation, multi-processing version
    ...
    if n_jobs == 1:
        ...
        for fid in fid_lst:
            ...
            kde = KernelDensity(kernel='gaussian', bandwidth=0.2).fit(x[fid])

            input_density = kde.score_samples(x[fid])
...
#
        ...
        bps_viz_encode_func = partial(viz_encode, bps_arrangement=bps_arrangement, n_bps_points=n_bps_points, 
                                  radius=radius, n_parts=n_parts,
                                  bps_cell_type=bps_cell_type, verbose=verbose, random_seed=random_seed,
                                  x_features=x_features, custom_basis=custom_basis, n_jobs=1)
        ...
        pool = multiprocessing.Pool(n_jobs)
        x_chunks = np.array_split(x, n_jobs)
        x_bps = np.concatenate(pool.map(bps_viz_encode_func, x_chunks), 0)
        pool.close()

        return x_bps
    
def fft_encode(x, bps_arrangement='random', n_bps_points=512, n_parts=8, radius=1.5, bps_cell_type='dists',
           verbose=1, random_seed=13, x_features=None, custom_basis=None, n_jobs=-1):
    """Converts point clouds to basis point set (BPS) representation, multi-processing version
    ...
    if n_jobs == 1:
        ...
        #fftx = np.linspace(0, 1, fft_grid)
        #ffty = np.linspace(0, 1, fft_grid)
        #fftz = np.linspace(0, 1, fft_grid)
#
        #xval, yval, zval = np.meshgrid(fftx,ffty,fftz)
#
        ##fftgrid = np.vstack((xval, yval, zval))
#
        #fftgrid = np.hstack((fftx,ffty,fftz))

        #print(xval)

        #fftgrid = np.reshape(-1, 3)

        #fft_grid = np.mgrid[0:2, 0:2, 0:2]
        #fft_grid = fft_grid.reshape(-1, 3)
        #print(fft_grid.shape)

        # fft_grid = utils.autogrid(np.array([[0, 0, 0]]), boundary_abs=1, num_points=13)

        for fid in fid_lst:
            # Compute kernel density estimate on a grid using Silverman's rule for bw
            #xcoord, input_density = FFTKDE(kernel="gaussian", bw=1E-10).fit(x[fid]).evaluate(fft_grid)
            
            xcoord, input_density = FFTKDE(kernel="gaussian", bw=1E-4).fit(x[fid]).evaluate(fft_grid+1)

            #fig, ax = plt.subplots(subplot_kw=dict(projection='3d'))
            #xx, yy, zz = fft_grid
            #vmin = min(input_density)
            #vmax = max(input_density)
            ## z y and inverted to show it on a different plane
            #ax.scatter(xx, zz, yy, c=input_density, vmin=vmin, vmax=vmax)
            ...
            input_density_cat = np.concatenate([xcoord, input_density], axis=1)
            ...
            for i in range(0, n_parts):
                ...
                # print("part: " + str(i) + " "+ str(n_curr_basis_points))
        ...
        bps_fft_encode_func = partial(fft_encode, bps_arrangement=bps_arrangement, n_bps_points=n_bps_points, 
                                  radius=radius, n_parts=n_parts,
                                  bps_cell_type=bps_cell_type, verbose=verbose, random_seed=random_seed,
                                  x_features=x_features, custom_basis=custom_basis, n_jobs=1)
        ...
        pool = multiprocessing.Pool(n_jobs)
        x_chunks = np.array_split(x, n_jobs)
        x_bps = np.concatenate(pool.map(bps_fft_encode_func, x_chunks), 0)
        pool.close()
